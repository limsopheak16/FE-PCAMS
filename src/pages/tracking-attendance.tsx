...
interface Student {
  ...
  englishName: string
  age: number
  status: "Present" | "Late" | "Absent" | "Status"
}
...
const StatusCell: React.FC<StatusCellProps> = ({ status, onChange }) => {
  ...
  const statusOptions: Student["status"][] = ["Present", "Late", "Absent", "Status"]

  return (
    <div className="relative">
      <button
        className={`${getStatusColor(status)} font-medium focus:outline-none`}
...
const TrackingAttendancePage = () => {
  const [students, setStudents] = useState<Student[]>([
    ...
    { id: "21775-4", khmerName: "ចាន់ សុវណ្ណា", englishName: "Sovanna Chan", age: 20, status: "Status" },
  ])
  ...
  const handleCreateNewChild = () => {
    console.log("Create new child clicked")
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 p-6">
        <h1 className="text-2xl font-bold mb-6">Tracking Attendance</h1>
        ...
        <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
          ...
          <div className="w-full md:w-auto">
            <label htmlFor="search" className="mb-1 font-medium block">
              Find Children
            </label>
            <div className="relative">
              <input
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">Khmer Name</th>
                <th className="px-6 py-3 text-left text-sm font-medium text-gray-700">English Name</th>
              ...
              {filteredStudents.map((student, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <StatusCell status={student.status} onChange={(newStatus) => handleStatusChange(index, newStatus)} />
                  </td>
                </tr>
              ))}
              {filteredStudents.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center text-gray-500 py-6">No students found.</td>
                </tr>
              )}
            </tbody>
          </table>
